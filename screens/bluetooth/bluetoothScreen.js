...
const BluetoothScreen = ({navigation}) => {
  ...
  const handleRealScreen = () => {
    navigation.navigate("BluetoothScreenReal");
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
            <TouchableOpacity style={{...styles.scanButton, marginTop: 63}} onPress={handleRealScreen}>
              <Text style={styles.buttonText}>{"Pantalla Real"}</Text>
            </TouchableOpacity>
          </View>
      {!scanning && (
        <FlatList
          ...
          ListEmptyComponent={() => (
            ...
            <Text style={styles.noDevicesText}>{"Esta es una pantalla de simulacion para expo app no compilado."}</Text>
          )}
        />
      )}
      {scanning ? (
